...
CUDA_VISIBLE_DEVICES=1 python3 -u /home/blu/jhu/codeswitch/v1.1/src/cslm/main.py \
    --model_name_or_path ${BLU_ARTIFACTS}/codeswitch/v1.1/mixture-of-softmax-exp72-$JOB_ID/checkpoint-${CHEKPOINT}/pytorch_model.bin \
    --encoder_config \
    ...
    --decode_do_sample \
    --decode_load_cache ${BLU_ARTIFACTS}/codeswitch/v1.1/mixture-of-softmax-exp72-$JOB_ID/checkpoint-${CHEKPOINT}/evaluations/${DECODE_MODE}/num-beams-${BEAM_SIZE}/predictions.${i}.json \
    --eval_mode \
      ${eval_mode} \
    --eval_output \
    ${BLU_ARTIFACTS}/codeswitch/v1.1/mixture-of-softmax-exp72-$JOB_ID/checkpoint-${CHEKPOINT}/evaluations/${DECODE_MODE}/num-beams-${BEAM_SIZE}/evaluations.${i}.bin${bin}.${eval_mode}.json \
    --eval_format data \
      --eval_filter "constrained_decoding_bin_selector(${bin})" "constrained_decoding_length_selector(5)"
done
shopt -s extglob
ROOT=${BLU_ARTIFACTS}/codeswitch/v1.1/mixture-of-softmax-exp72-$JOB_ID/checkpoint-${CHEKPOINT}/evaluations/${DECODE_MODE}/num-beams-${BEAM_SIZE}
