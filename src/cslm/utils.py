...
def seq_numel(s1):
    ...
    for m1 in s1:
        if m1 is None:
            logger.warning("skipping None tensors in seq_numel")
            continue
        cum += m1.numel()
    return cum
...
def seq_sum(s1):
    ...
    for m1 in s1:
        if m1 is None:
            logger.warning("skipping None tensors in seq_sum")
            continue
        cum += m1.sum()
    return cum

def seq_scale(s1, scale):
    if not isinstance(s1, tuple):
        raise ValueError(f"{len(s1)}")
    results = tuple()
    for m1 in s1:
        if m1 is None:
            logger.warning("skipping None tensors in seq_scale")
            continue
        results += (m1 * scale,)
    return results
...
def seq_sub(s1, s2):
    if s1 is None: return seq_scale(s2, -1)
    if s2 is None: return s1
    if not isinstance(s1, tuple) or not isinstance(s2, tuple) or len(s1) != len(s2):
        raise ValueError(f"{len(s1)}, {len(s2)}")
    results = tuple()
...
def seq_mul(s1, s2):
    ...
    for m1, m2 in zip(s1, s2):
        if m1 is None and m2 is None:
            logger.warning("skipping paired None tensors in seq_mul")
            continue
        assert m1.size() == m2.size()
        results += (m1 * m2,)
    return results
