...
def assign_bin_factory():
    def assign_bin(prefix_state):
        switch_percentage = torch.where(prefix_state["fence_count"] == 0, 0.0 * prefix_state["fence_count"],  prefix_state["switch_count"]/ prefix_state["fence_count"])
        null_switch = switch_percentage == 0
        bin_1 = (0.0 < switch_percentage) & (switch_percentage <= 1/5)
        bin_2 = (1/5 < switch_percentage) & (switch_percentage <= 2/5)
        bin_3 = (2/5 < switch_percentage) & (switch_percentage < 3/5)
        bin_4 = (3/5 <= switch_percentage) & (switch_percentage < 4/5)
        bin_5 = (4/5 <= switch_percentage) & (switch_percentage < 1.0)
